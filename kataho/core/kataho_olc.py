...
PAIR_RESOLUTIONS_ = [20.0, 1.0, .05, .0025, .000125, 0.00000625]
...
def decode(code):
    ...
    return CodeArea(
        round(lat, 14), round(lng, 14), 
        round(lat + latPrecision, 14), round(lng + lngPrecision, 14),
...
def shorten(code, latitude, longitude):
    ...
    if codeArea.codeLength < MIN_TRIMMABLE_CODE_LEN_:
        raise ValueError('Code length must be at least ' + str(MIN_TRIMMABLE_CODE_LEN_))
    # Ensure that latitude and longitude are valid.
    latitude = clipLatitude(latitude)
    longitude = normalizeLongitude(longitude)
...
class LatLong:
    ...
    def to_sting(self):
        return f"LatLong({self.latitude}, {self.longitude})"